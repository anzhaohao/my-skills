...
Use the configured image-edit backend once to remove text only. The cleaned reference must retain arrows and arrow tails, connectors, frames, axes, heatmaps, legends, scientific subjects, colors, spacing, and layout. Do not split a complete reference into mandatory per-subject uploads.
