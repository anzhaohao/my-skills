---
name: cell-lct
description: Create, reconstruct, and append editable scientific vector figures with live editable text in the user's currently open Adobe Illustrator document. Use for scientific subjects, mechanism diagrams, workflows, graphical abstracts, review figures, reference-image recreation, and continued drawing while preserving existing artwork. Harness-neutral: works through the configured image-edit backend, vectorize provider, and the cell-lct CLI.
---
...
`text manifest -> configured image-edit backend (text-only cleanup) -> complete-reference vectorization -> live text merged into master SVG -> one-time geometry cache -> persistent Illustrator playback`
...
The active harness adapter is documented in [references/harness.md](references/harness.md) (written by the installer for your harness; fall back to the generic instructions when absent).

All execution goes through the stable CLI entry `scripts/cell-lct.ps1`:

- `cell-lct.ps1 doctor` — environment diagnostics
- `cell-lct.ps1 verify` — vectorize-provider authentication probe
- `cell-lct.ps1 image-clean -InputImage <path> -OutputImage <path> [-Provider <name>]` — text-only cleanup through the configured image-edit backend
- `cell-lct.ps1 vectorize -InputImage <path> -OutputSvg <path>` — true-vector conversion through the configured vectorize provider
- `cell-lct.ps1 prepare -InputSvg <path> -WorkDir <path>` — one-time geometry cache
- `cell-lct.ps1 draw -InputSvg <path> -WorkDir <path> [-DryRun]` — Illustrator playback
- `cell-lct.ps1 reconstruct -InputImage <path> -TextManifest <path> -OutputRoot <path> [-DryRun]` — full pipeline

Do not call internal runtime scripts directly unless the CLI proves insufficient.
...
- If the API key is not configured, run `scripts/set-xiaomiao-key.ps1` in an interactive terminal and accept it through hidden input only. The key is stored under the neutral Cell-lct home (`~/.cell-lct/secrets/`) with Windows DPAPI; legacy `~/.codex/secrets` files remain readable as a fallback.
- Verify authentication with `scripts/cell-lct.ps1 verify` before the first paid request.
- Keep the key in Windows DPAPI storage only. Never place it in any project, command line, repository file, log, task output, cache, or delivered artifact.
...
3. Use the configured image-edit backend to remove text only from the complete reference. Preserve arrows and arrow tails, connectors, frames, coordinate axes, heatmaps, legends, scientific subjects, colors, spacing, and the complete layout.
4. Send the complete text-cleaned reference through the configured vectorize provider. Do not force per-subject uploads for a complete reference figure.
...
7. For an already approved true-vector SVG, validate it and run `scripts/cell-lct.ps1 draw` directly.

## Reconstruction contract

1. Preserve the untouched reference and create the complete text manifest before cleanup.
2. The image-edit backend may remove text only. It must not delete, rebuild, move, or restyle any non-text structure.
3. Obtain a fresh complete true-vector SVG through the configured vectorize provider. Local raster tracing is forbidden.
...
7. Parse the complete Master SVG exactly once with `scripts/cell-lct.ps1 prepare`.
...
Use the next available basename for all generated, downloaded, intermediate, and final deliverables: `shibielujing1`, `shibielujing2`, `shibielujing3`, ... Allocate it with `scripts/cell-lct.ps1 prepare` (or the underlying allocator when the CLI is unavailable).
...
- the text manifest is complete and the image-edit backend removed text only;
