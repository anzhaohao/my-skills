...
# Harness-neutral resolution: explicit arguments > env > config.json >
# neutral defaults > legacy Codex fallback.
. (Join-Path $PSScriptRoot "resolve-config.ps1") -HomeRoot $HomeRoot -SecretPath $SecretPath -XiaomiaoBaseUrl $BaseUrl
$root = ([string]$merged.xiaomiao_base_url).TrimEnd('/')
...
function Get-XiaomiaoToken {
    ...
    if ($resolved.source -eq "missing") {
        throw "CELL_LCT_SECRET_MISSING|$($resolved.path)|Run set-xiaomiao-key.ps1 first."
    }
    return Read-CellLctSecret -SecretPath $resolved.path
}
...
switch ($Action) {
    ...
    "verify" {
        ...
        try {
            $probeId = "__cell_lct_connection_probe__"
