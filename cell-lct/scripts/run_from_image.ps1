...
$allocator = Resolve-RuntimeFile -Name "allocate_shibielujing_name.py" -SubDir "python"
$runner = Resolve-RuntimeFile -Name "run_cell_lct.ps1"
$vectorizer = Resolve-RuntimeFile -Name "vectorize-xiaomiao.ps1"
$textMerger = Resolve-RuntimeFile -Name "merge_live_text.py" -SubDir "python"
...
& $vectorizer -InputImage $inputPath -OutputSvg $rawSvg
if (-not $?) { throw "Vectorization failed." }
...
& $runner @arguments
if (-not $?) { throw "Illustrator playback failed." }
