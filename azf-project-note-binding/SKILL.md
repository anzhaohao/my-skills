...
description: Bind An Zhaofeng's code projects to Obsidian project notes and keep code-understanding notes synchronized with code changes. Use when the user asks to bind a repository/project to Obsidian, create or update .azf/project-notes.yaml, sync notes after code edits, update code-understanding notes, maintain project handoff notes from code diffs, or asks "代码改了笔记怎么同步", "项目和笔记绑定", "同步项目笔记".
---

# AZF Project Note Binding

Use this skill to connect a code project with its Obsidian project folder, then maintain the notes when the code changes. Pair it with `azf-obsidian-workflow` for note structure and `azf-deep-learning-experiment-record` when the code change involves training artifacts or metrics.
...
- Obsidian `项目理解/` structure and main project/code understanding index.
...
notes:
  main: ISFC模型仿真复现工作流.md
  handoff: 项目进度.md
  project_understanding_map: 项目理解/00_项目理解总图.excalidraw
  project_understanding_index: 项目理解/01_项目理解索引.md
  file_graph_dir: 项目理解/文件结构图
  experiment_dir: 实验记录
  excalidraw: ISFC项目总图.excalidraw

sync:
  mode: manual
  last_sync: null
  update_policy:
    main_note: status_only
...
Use native `.excalidraw` files for project master maps unless An Zhaofeng explicitly asks for `.excalidraw.md`.
...
```text
项目理解/
  00_项目理解总图.excalidraw
  01_项目理解索引.md
  文件结构图/
    <filename>结构.excalidraw
```

In this workflow, `项目理解` means code-first project understanding. Do not create a separate `代码理解/` folder unless An Zhaofeng explicitly asks for it. The master file `00_项目理解总图.excalidraw` is the code/project understanding overview map: it should look like a clean code flow map, not a project-management dashboard, background note, or concept encyclopedia. Markdown in this folder should stay thin and serve as an index only.
...
- The master map should present a simple, memorable logic first. Avoid adding every support file as a visible node if it makes the core code story harder to read; secondary files can stay in `文件结构图/` and be linked from the index.
...
```text
项目理解/文件结构图/<filename>结构.excalidraw
```
...
5. Ensure the note folder has:
   - main workflow note
   - progress handoff note or equivalent
   - `项目理解/00_项目理解总图.excalidraw`
   - `项目理解/01_项目理解索引.md`
   - `项目理解/文件结构图/`
6. Add a short reciprocal link in the main workflow note pointing to the code index.
...
4. Update only affected notes:
   - `entrypoint` -> `01_项目运行入口.md`
   - `data_flow` -> `02_整体数据流.md`
   - `model_or_core` -> `03_核心模块关系.md` and `04_关键类与函数.md`
   - `config` -> `05_配置文件与参数.md`
   - `experiment` -> `实验记录/` and the progress handoff
5. Always create or append one dated sync record:

```text
项目理解/代码变更同步记录/YYYYMMDD_代码变更同步.md
```
