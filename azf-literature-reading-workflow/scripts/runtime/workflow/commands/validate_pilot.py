...
from workflow.services.artifact_runs import latest_successful_state_dir, promote_artifact_run, state_file, workspace_with_artifacts
...
def run(args) -> int:
    ...
    for workspace in workspaces:
        ...
        paper = workspace_with_artifacts(workspace, locations.get("artifact_root"))
        external_state = latest_successful_state_dir(locations.get("artifact_root"), workspace)
        source_anchor = paper.source_anchor_path if paper.source_anchor_path.is_file() else state_file(external_state, "source-anchors.json")
        ...
        if not issues and paper.artifact_root and paper.artifact_id:
            promote_artifact_run(paper.artifact_root, workspace, paper.artifact_id)
        results.append(
            {
