...
def _make_vault(tmp_path: Path, concept_relative: Path, *, marker: bool = True, base_relative: str | None = None) -> dict[str, Path]:
    ...
    (concept_root / "相位恢复.md").write_text(
        f"---\n类型: 概念卡\n{marker_lines}---\n\n# 一句话解释\n\n恢复相位。\n",
        encoding="utf-8",
    )
...
def test_resolver_discovers_moved_flat_concept_library(tmp_path: Path) -> None:
    paths = _make_vault(
        tmp_path,
        Path("02-Brain Cells/99_扫盲班"),
    )
    registry = tmp_path / "locations.yaml"
    stale = paths["vault_root"] / "02-Brain Cells" / "99_扫盲班_旧位置"
    registry.write_text(
        yaml.safe_dump(
            {
                "locations": {
    ...
    assert result.sources["concept_library_root"] == "concept-card-directory-discovery"
    assert result.locations["artifact_root"] == paths["artifact_root"].resolve()
    assert result.sources["artifact_root"] == "approved-registry"
    ...
    assert not any(path.is_dir() for path in paths["concept_library_root"].iterdir())
    assert (paths["concept_library_root"] / "相位恢复.md").is_file()
...
def test_confirmed_manifest_rejects_folder_move(tmp_path: Path) -> None:
    paths = _make_vault(tmp_path, Path("02-Brain Cells/99_扫盲班"))
